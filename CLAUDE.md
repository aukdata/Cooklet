...
**ESLint抑制コメント（`// eslint-disable`）は決して使用してはいけません。**根本的な型安全性とコード品質の改善で解決してください。
適宜`pnpx eslint . && pnpm run build`を実行して、コードの品質を維持してください。
**【重要】コミット前の必須チェック項目**：
- 必ず`pnpx eslint .`でESLintエラーが0件であることを確認してください。
- 必ず`pnpm run build`でビルドが成功することを確認してください。
- 上記2つのチェックに失敗した場合は、コミット・プッシュを行わず、エラーを修正してから再度実行してください。
...
- サマリー画面（動的データ表示・期間別アラート）- issue #10完了
- カレンダー画面（献立表示・週間ビュー・完全実装）
- コスト管理画面（支出記録・月間サマリー）
- レシピ管理画面（完全実装・献立追加機能）- issue #31完了
- 在庫管理画面（完全実装）
- 買い物リスト画面（完全実装）
- タブナビゲーション（6タブ構成）- issue #11完了
- データベーススキーマ（Supabase）
- **共通UIコンポーネント（Button.tsx）** - issue #16完了
- **編集ボタンの統一化** - 在庫画面ベースのデザイン統一（issue #15, #12対応）
- **削除ボタンのリスト非表示化** - ダイアログ内のみに制限（issue #15対応）
- **レシピ「献立に追加」ボタン機能** - 日付・食事タイプ選択ダイアログ（issue #31完了）
- **レシピ最後に作った日表示** - MM/DD形式での表示（issue #31完了）
- **献立画面のレシピリンク表示統一** - 「レシピを見る」→「レシピ」（issue #30完了）
